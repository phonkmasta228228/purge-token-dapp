...
// Returns days/hours remaining inside the 14-day penalty-free window (null = no penalty risk)
function getPenaltyWindowInfo(maturityTs: bigint): { daysLeft: number; urgent: boolean } | null {
  const now = BigInt(Math.floor(Date.now() / 1000));
  if (now < maturityTs) return null; // not yet mature
  const windowEnd = maturityTs + PENALTY_WINDOW_SECS;
  if (now >= windowEnd) return null; // window already expired (slot uncollectable)
  const secsLeft = Number(windowEnd - now);
...
const PENALTY_WINDOW_SECS = BigInt(14 * 86400); // 14-day claim window after maturity
      ...
      for (let i = 0; i < pdas.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          for (let j = 0; j < infos.length; j++) {
            ...
            if (mint.active && nowTs < mint.maturityTs + PENALTY_WINDOW_SECS) activeMints.push(mint);
          }
        } catch {
          ...
          {mints.slice(0, visibleCount).map((mint) => {
            ...
            return (
              ...
              >
                ...
                {isMature && (() => {
                  const pw = getPenaltyWindowInfo(mint.maturityTs);
                  return pw ? (
                    <div className={`mb-3 flex items-center gap-2 rounded px-3 py-2 text-xs font-bold tracking-widest ${
                      pw.urgent
                        ? 'bg-[#1a0500] border border-orange-700 text-orange-400'
                        : 'bg-[#0d1a0d] border border-[#00FFAA33] text-[#00FFAA]'
                    }`}>
                      {pw.urgent ? '🚨' : '⏰'}
                      <span>
                        {pw.daysLeft === 1
                          ? 'Last day to claim penalty-free!'
                          : `${pw.daysLeft} days left to claim penalty-free`}
                      </span>
                    </div>
                  ) : null;
                })()}
